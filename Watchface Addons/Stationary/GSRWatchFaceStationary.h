#include "StationaryText.h"

RTC_DATA_ATTR int MyGSRStationaryWatchFaceId; // Watchface ID #.
// Approx. 3035 bytes

class GSRWatchFaceStationary : public WatchyGSR
{
public:
  GSRWatchFaceStationary() : WatchyGSR() { initAddOn(this); } // *** DO NOT EDIT THE CONTENTS OF THIS FUNCTION ***

  WatchyStationary StationaryWatchFace;

  void RegisterWatchFaces()
  { // Add WatchStyles here (this is done pre-boot, post init)
    MyGSRStationaryWatchFaceId = AddWatchStyle("Stationary", this);
  };
  ...
  void InsertDrawWatchStyle(uint8_t StyleID)
  {
    uint8_t X, Y;
    uint16_t A;
    if (StyleID == MyGSRStationaryWatchFaceId)
    {
      if (SafeToDraw())
      {
        StationaryWatchFace.drawWatchFace();
      }
    }
  };
};

GSRWatchFaceStationary MyGSRWatchFaceStationary;
